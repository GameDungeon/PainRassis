...
#define FOLD_DATA(sc, de, fl, mv) ( (sc + INF_BOUND) | (de << 16) | (fl << 23)  | ((uint64_t)mv << 25))
...
void DataCheck(const int move) {
	...
	uint64_t data = FOLD_DATA(score, depth, flag, move);
...
void VerifyEntrySMP(S_HASHENTRY *entry) {
    /*
	uint64_t data = FOLD_DATA(entry->score, entry->depth, entry->flags, entry->move);
	uint64_t key = entry->posKey ^ data;
...
int ProbeHashEntry(S_BOARD *pos, S_HASHTABLE *table, int *move, int *score, int alpha, int beta, int depth) {
	...
	uint64_t test_key = pos->posKey ^ table->pTable[index].smp_data;
...
void StoreHashEntry(S_BOARD *pos, S_HASHTABLE *table, const int move, int score, const int flags, const int depth) {
	...
	uint64_t smp_data = FOLD_DATA(score, depth, flags, move);
	//uint64_t smp_key = pos->posKey ^ smp_data;
